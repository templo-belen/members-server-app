...
def test_create_general_data__with_member_has_already_general_data__returns_409(client):
    create_response = client.post("/members/1/general-data", json=member_creation_json,
                                  headers=get_auth_headers(client, "admin", "12345"))
    assert create_response.status_code == 409

def test_create_general_data__with_member_not_found__returns_404(client):
    create_response = client.post("/members/1000/general-data", json=member_creation_json,
                                  headers=get_auth_headers(client, "admin", "12345"))
    assert create_response.status_code == 404

def test_create_general_data__with_member_ok__returns_200(client):
    create_response = client.post("/members/101/general-data", json=member_creation_json,
                                  headers=get_auth_headers(client, "admin", "12345"))
...
def test_update_general_data__with_member_not_found__returns_404(client):
    create_response = client.put("/members/1000/general-data", json=member_updating_json,
                                 headers=get_auth_headers(client, "admin", "12345"))
    assert create_response.status_code == 404
...
def test_update_general_data__with_member_without_general_data__returns_404(client):
    ...
    create_response = client.put("/members/101/general-data", json=without_general_data_json,
                                 headers=get_auth_headers(client, "admin", "12345"))
    assert create_response.status_code == 404

def test_update_general_data__with_member_ok__returns_200(client):
    create_response = client.put("/members/1/general-data", json=member_updating_json,
                                 headers=get_auth_headers(client, "admin", "12345"))
    assert create_response.status_code == 200
