...
class MembersReferenceService:
    ...
    def update_member_reference(
        self, member_id: int, reference_data: UpdateMemberReferenceRequest, db: Session
        ...
        reference_to_update = (
            ...
            .filter(MembersReference.id == reference_data.id, MembersReference.member_id == member_id)
            .first()
        )
        if not reference_to_update:
            raise NotFoundException(
                f"Los datos de referencia con id {reference_data.id} del miembro " f"con id {member_id} no existen."
            )
