...
class MembersDEWService:
    ...
    def update_member_dew(self, dew_data: UpdateMemberDEWRequest, db: Session) -> MembersDEWResponse:
        member_to_update = db.query(Member).filter(Member.id == dew_data.member_id).first()
